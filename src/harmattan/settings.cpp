...
QColor Settings::activeColor() {
    return value("activeColor", "#0881cb").value<QColor>();
}

void Settings::setActiveColor(const QColor &color) {
    if (color != activeColor()) {
        setValue("activeColor", color);
...
QString Settings::activeColorString() {
    return value("activeColorString", "color7").toString();
}

void Settings::setActiveColorString(const QString &string) {
    if (activeColorString() != string) {
        setValue("activeColorString", string);
